...
const MATCH_STRENGTH = { None: 'none', Weak: 'weak', Moderate: 'moderate', Strong: 'strong' } as const;
function getMatchStrength(score: number | undefined | null, max = 100): keyof typeof MATCH_STRENGTH {
...
/** Half-circle gauge for match score (Financial Score style) */
function MatchScoreGauge({ value, max, uniqueId }: { value: number | null; max: number; uniqueId: string }) {
  const strokeRef = useRef<SVGCircleElement>(null);
  const radius = 45;
  const circumference = 2 * Math.PI * radius;
  const half = circumference / 2;
  const strokeDasharray = `${half} ${half}`;
  const dashOffset = value != null ? -Math.min((value / max), 1) * half : -half / 4;
  const strength = getMatchStrength(value, max);
  const strengthColors: Record<string, string[]> = {
    none: ['#e2e8f0', '#cbd5e1'],
    weak: ['#fda4af', '#f43f5e', '#be123c'],
    moderate: ['#fde68a', '#f59e0b', '#b45309'],
    strong: ['#6ee7b7', '#10b981', '#047857'],
  };
  const colorStops = strengthColors[strength] ?? strengthColors.none;
  const gradId = `job-grad-${uniqueId}`;

  useEffect(() => {
    if (!strokeRef.current || value == null) return;
    strokeRef.current.animate(
      [{ strokeDashoffset: '0', offset: 0 }, { strokeDashoffset: String(dashOffset) }],
      { duration: 1200, easing: 'cubic-bezier(0.65, 0, 0.35, 1)', fill: 'forwards' }
    );
  }, [value, max, dashOffset]);

  return (
    <svg className="block mx-auto w-full h-32" viewBox="0 0 100 60" aria-hidden="true">
      <defs>
      ...
      <g fill="none" strokeWidth={9} transform="translate(50, 50)" strokeLinecap="round">
        <circle className="stroke-slate-100 dark:stroke-slate-800" r={radius} strokeDasharray={strokeDasharray} transform="rotate(180)" />
        <circle ref={strokeRef} stroke={`url(#${gradId})`} strokeDasharray={strokeDasharray} r={radius} transform="rotate(180)" style={{ strokeDashoffset: dashOffset }} />
      </g>
    </svg>
  );
}

function JobCardGlass(props: {
  job: any;
  ...
  getStrengthBadgeClass: (s: string) => string;
  isAnalyzingJob: boolean;
  selectedJobForAnalysis: any;
  resumeData: any;
  onTrackJob: (job: any) => void;
  onApplyAndTrack: (job: any) => void;
  onAnalyzeJob: (job: any) => void;
  ...
  const cardId = `job-${job.id ?? job.title ?? Math.random().toString(36).slice(2)}`;

  const descriptionSnippet = (job.description || '').slice(0, 120) + ((job.description || '').length > 120 ? '…' : '');
  const backFeatures = [
    job.company && `Company: ${job.company}`,
    job.location && `Location: ${job.location}`,
    job.salary && `Salary: ${job.salary}`,
    job.whyMatch && `Match: ${job.whyMatch}`,
  ].filter(Boolean) as string[];

  return (
    <div
      className="group relative h-[420px] w-full max-w-sm min-w-[320px] [perspective:2000px] animate-in fade-in slide-in-from-bottom-4 duration-500 fill-mode-both"
    ...
    >
      ...
      >
        ...
        <div className="absolute inset-0 h-full w-full [backface-visibility:hidden] [transform:rotateY(0deg)]">
          <div
            className={cn(
              'h-full w-full flex flex-col gap-3 rounded-[2.5rem] border border-slate-200/60 py-6 px-5 overflow-hidden',
              'bg-white/80 backdrop-blur-2xl shadow-xl shadow-slate-200/40',
              'transition-all duration-500 group-hover:shadow-2xl group-hover:border-indigo-500/20'
            )}
          >
            <div className="absolute inset-0 -z-10 opacity-30 pointer-events-none" style={{ filter: 'url("#container-glass")' }} />
            <div className="flex items-start justify-between gap-3 pb-2 px-1">
              <h3 className="text-lg font-bold tracking-tight text-slate-800 dark:text-slate-100 leading-tight line-clamp-2">{job.title}</h3>
              {strength !== 'none' && (
                <span className={cn('shrink-0 inline-flex items-center rounded-full border px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-widest', getStrengthBadgeClass(strength))}>
                  {strength}
                </span>
              )}
            </div>
            <div className="relative min-h-[100px] flex items-center justify-center px-2">
              <div className="absolute inset-0 bg-indigo-500/5 blur-3xl rounded-full scale-75 group-hover:scale-90 transition-transform duration-1000" />
              <MatchScoreGauge value={matchScore} max={maxScore} uniqueId={cardId} />
              <div className="absolute bottom-2 left-0 w-full text-center pointer-events-none">
                <span className="text-2xl font-black text-slate-900 dark:text-white">
                  {hasScore ? `${matchScore}` : '—'}
                </span>
                <span className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest">match</span>
              </div>
            </div>
            <div className="flex items-center gap-2 text-xs text-slate-500 flex-wrap px-1">
            ...
            <p className="text-slate-500 text-sm leading-relaxed line-clamp-2 px-1 flex-1 min-h-0">{descriptionSnippet}</p>
            <div className="px-1 pt-2">
              <button
                type="button"
                className={cn(
                  'w-full inline-flex items-center justify-center gap-2 rounded-full h-12 px-6 text-sm font-bold transition-all',
                  'bg-indigo-600 text-white hover:scale-[1.02] active:scale-[0.98] shadow-lg shadow-indigo-200',
                  'border border-white/20'
                )}
                onClick={(e) => { e.stopPropagation(); setIsFlipped(true); }}
              >
                View details <ArrowRight className="w-4 h-4 opacity-70" />
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="absolute inset-0 h-full w-full [backface-visibility:hidden] [transform:rotateY(180deg)]">
          <div
            className={cn(
              'h-full w-full flex flex-col rounded-[2.5rem] border border-indigo-100 py-6 px-5 overflow-hidden',
              'bg-gradient-to-br from-indigo-50/90 via-white to-white dark:from-indigo-950/30 dark:via-slate-900 dark:to-slate-900 backdrop-blur-2xl shadow-2xl shadow-indigo-100/50'
            )}
          >
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h3 className="text-base font-bold text-slate-900 dark:text-slate-100 leading-tight">Job Details</h3>
                <p className="text-[10px] uppercase tracking-widest font-bold text-indigo-500">Actions & match</p>
              </div>
            </div>
            {job.whyMatch && (
              <div className="mb-3 p-3 bg-indigo-50/80 dark:bg-indigo-900/20 rounded-xl border border-indigo-100 dark:border-indigo-800">
                <p className="text-xs font-medium text-slate-700 dark:text-slate-300"><span className="font-bold text-indigo-600 dark:text-indigo-400">Why this matches:</span> {job.whyMatch}</p>
              </div>
            )}
            <div className="space-y-2 flex-1 overflow-y-auto min-h-0">
              {backFeatures.slice(0, 4).map((feature, i) => (
                <div key={i} className="flex items-start gap-2">
                  <div className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-md bg-indigo-100 dark:bg-indigo-900/50">
                    <CheckCircle2 className="h-3 w-3 text-indigo-600 dark:text-indigo-400" />
                  </div>
                  <span className="text-xs font-medium text-slate-600 dark:text-slate-400 leading-snug line-clamp-2">{feature}</span>
                </div>
              ))}
            </div>
            {isTracked && (
              <div className="mb-3 p-2 bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-100 dark:border-emerald-800 rounded-xl flex items-center gap-2">
                <Check className="w-4 h-4 text-emerald-600" />
                <span className="text-xs font-bold text-emerald-800 dark:text-emerald-200">Already tracked</span>
              </div>
            )}
            <div className="mt-auto space-y-2 pt-4 border-t border-indigo-50 dark:border-indigo-900/50">
              {isTracked ? (
                <button type="button" onClick={(e) => { e.stopPropagation(); onNavigateToTracker(); }} className="w-full flex items-center justify-center gap-2 py-3 rounded-2xl bg-indigo-600 text-white text-sm font-bold hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-100">
                  <Check className="w-4 h-4" /> View in Tracker
                </button>
              ) : (
                <>
                  <button type="button" onClick={(e) => { e.stopPropagation(); onTrackJob(job); }} className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-200 text-sm font-bold hover:bg-slate-50 dark:hover:bg-slate-700 transition-all">
                    <BookmarkPlus className="w-4 h-4" /> Track This Job
                  </button>
                  <button type="button" onClick={(e) => { e.stopPropagation(); onApplyAndTrack(job); }} className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-200 text-sm font-bold hover:bg-slate-50 dark:hover:bg-slate-700 transition-all">
                    <Check className="w-4 h-4" /> Apply & Track
                  </button>
                </>
              )}
              <button type="button" onClick={(e) => { e.stopPropagation(); onAnalyzeJob(job); }} disabled={isAnalyzingJob || !resumeData} className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-sm font-bold hover:bg-slate-50 dark:hover:bg-slate-700 transition-all disabled:opacity-50 disabled:cursor-not-allowed">
              ...
              <a href={job.url} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} className="flex items-center justify-between gap-2 p-3 rounded-2xl bg-indigo-600 text-white cursor-pointer hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-100 text-sm font-bold">
                <span>Apply Now</span>
...
const JobFinder: React.FC<JobFinderProps> = ({ onViewChange, initialSearchTerm }) => {
  ...
  const getStrengthBadgeClass = (strength: string) => {
    switch (strength) {
      case 'weak': return 'bg-red-50 text-red-500 border-red-100';
  ...
  // Job card with glass + 3D flip (Financial Score style), using our data and handlers
  const renderJobCard = (job: any, index: number) => {
    const matchScore = job.matchScore != null ? Number(job.matchScore) : null;
    const strength = getMatchStrength(matchScore);
    ...
    return (
      <JobCardGlass
        ...
        getStrengthBadgeClass={getStrengthBadgeClass}
        isAnalyzingJob={isAnalyzingJob}
        selectedJobForAnalysis={selectedJobForAnalysis}
        resumeData={resumeData}
        onTrackJob={handleTrackJob}
        onApplyAndTrack={handleApplyAndTrack}
        onAnalyzeJob={handleAnalyzeJob}
        onNavigateToTracker={() => navigateTo('/dashboard/tracker')}
      />
    );
  };
        ...
        <div className="space-y-8">
          ...
          {jobResults.length > 0 ? (
            <div className="space-y-6 animate-fade-in-up">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 justify-items-center">{jobResults.map((job, index) => renderJobCard(job, index))}</div>
            </div>
          ) : (
