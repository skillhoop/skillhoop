...
const WorkflowPrompt = ({ message, onDismiss, onAction, actionText }: any) => (
  <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-50 bg-emerald-50/95 text-emerald-900 p-4 rounded-xl shadow-xl flex items-center gap-4 animate-fade-in-up border border-emerald-200">
    <span className="font-medium text-sm">{message}</span>
    <div className="flex gap-2">
        <button
          onClick={() => onAction('continue')}
          className="bg-emerald-600 text-white hover:bg-emerald-700 px-4 py-1.5 rounded-lg text-xs font-bold transition-colors shadow-sm"
        >
          {actionText}
        </button>
        <button
...
const JobFinder: React.FC<JobFinderProps> = ({ onViewChange, initialSearchTerm }) => {
  ...
  const renderJobCard = (job: any, index: number) => (
    <div key={index} className="bg-gradient-to-br from-slate-50 via-white to-purple-50 border border-slate-200 rounded-2xl p-6 hover:shadow-md transition-all group">
      <div className="flex items-start justify-between">
        <div className="flex-1">
          ...
          <div className="flex flex-wrap gap-3 mt-4 pt-4 border-t border-slate-100">
            ...
            <button onClick={(e) => { e.stopPropagation(); handleAnalyzeJob(job); }} disabled={isAnalyzingJob || !resumeData} className="px-4 py-2 bg-slate-50 text-slate-700 border border-slate-200 rounded-xl text-sm font-bold hover:bg-slate-100 hover:text-neutral-900 transition-all flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed">
            ...
            <a href={job.url} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} className="ml-auto px-6 py-2 bg-slate-900 text-white rounded-xl text-sm font-bold hover:bg-slate-800 transition-all flex items-center gap-2 shadow-sm">
  ...
  return (
    <div className="space-y-6">
      {notification && (
        <div
          className={`fixed top-4 right-4 z-50 px-6 py-4 rounded-xl shadow-xl border animate-slide-in ${
      ...
      <div className="flex items-center gap-2 p-2 rounded-2xl border bg-slate-100 border-slate-200 shadow-sm overflow-x-auto">
        {[
      ...
      {activeTab === 'search' && (
        <div className="space-y-8">
          <div className="bg-gradient-to-br from-slate-50 via-white to-purple-50 border border-slate-200 rounded-2xl p-8 relative shadow-sm">
